...
public class NodeFileStoreAPI extends JerseyResource implements NodeFileStoreApis {
    ...
    @Inject
    public NodeFileStoreAPI(CoreContainer coreContainer, DistribFileStore fileStore, SolrQueryRequest req, SolrQueryResponse rsp) {
    ...
    public ReadFileStoreResponse readFileStoreEntry(String path, Boolean sync, String getFrom, Boolean meta) {
        ...
        if (BooleanUtils.isTrue(meta)) {
            if (typ == FileStore.FileType.FILE) {
                ...
                response.files = Collections.singletonMap(path, l.isEmpty() ? null : l.get(0));
                return response;
            }
        } else {
